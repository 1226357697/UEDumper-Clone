...
static bool WideStringToUTF8(const wchar_t* wideStr, char* buf, int* slen) {
  ...
  WideCharToMultiByte(CP_UTF8, 0, wideStr, -1, buf, utf8Length, nullptr, nullptr);
  buf[utf8Length - 1]='\0';// ¼õÈ¥½áÎ²µÄ null ×Ö·û
  if(slen) *slen = utf8Length -1;
  return true;
}
...
std::string UE_FNameEntry::String(bool wide, uint16 len) const {
  char buf[0x1000];
  int slen = String(buf, wide, len);
  std::string name(buf, slen);
  return name;
}

int UE_FNameEntry::String(char *buf, bool wide, uint16 len) const {
  int slen = 0;
  if (wide) {
    wchar_t wbuf[1024]{};
    Read(object + offsets.FNameEntry.HeaderSize, wbuf, len * 2ull);
    /*if (Decrypt_WIDE) { Decrypt_WIDE(wbuf, len); }*/
    if (!WideStringToUTF8(wbuf, buf, &slen))
    {
...
bool UE_UPackage::Save(const fs::path &dir, bool spacing) {
  ...
  if (Classes.size()) {
    File file(dir / (packageName + "_classes.h"), "wb");
    if (!file) {
      return false;
    }
    if (spacing) {
  ...
  if (Structures.size() || Enums.size()) {
    File file(dir / (packageName + "_struct.h"), "wb");
    if (!file) {
      return false;
    }
